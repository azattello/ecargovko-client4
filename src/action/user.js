...
export const getUsersByFilial = async (filialText) => {
    try {
      const response = await fetch(`/api/user/users?filterByFilial=${filialText}`); // Исправил URL на правильный эндпоинт
      const data = await response.json();
      return data;
    } catch (error) {
      console.error('Ошибка при получении пользователей по филиалу:', error);
      throw error;
    }
  };
